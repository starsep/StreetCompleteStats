...
object BikeParkingCapacityQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "nwa[amenity=bicycle_parking][access!~'(private|no)'][bicycle_parking!=floor]")
...
object BikeParkingCoverQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "nwa[amenity=bicycle_parking][access!~'(private|no)'][bicycle_parking!~'(shed|lockers|building)']")
...
object BikeRentalCapacityQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "nwa[amenity=bicycle_rental][access!~'(private|no)'][bicycle_rental=docking_station]")
...
abstract class BusStopQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "nwa[public_transport=platform],nwa[highway=bus_stop][public_transport!=stop_position][physically_present!=no]")
}
...
object DietHalalQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features
          .select(
              "na[amenity~'(restaurant|cafe|fast_food|ice_cream|food_court)'][food!=no],na[shop~'(butcher|supermarket|ice_cream|convenience)']")
...
object BuildingLevelQuest : Quest {
  ...
  private val BUILDINGS_WITH_LEVELS =
      arrayOf(
          "house",
          "residential",
          "apartments",
  ...
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "a[building~'(${BUILDINGS_WITH_LEVELS.joinToString("|")})'][!man_made][location!=underground][ruins!=yes]")
...
object BuildingType : Quest {
  ...
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "a[building]${forbiddenTags.joinToString("") { "[!$it]" }}[location!=underground][ruins!=yes]")
...
object DefibrillatorIndoorQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select("n[emergency=defibrillator][access!~'(private|no)'][!location]")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select("*[indoor~'.*']")
...
object ToiletAvailabilityQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "na[shop~'(mall|department_store)'],nw[highway~'(services|rest_area)'],nw[tourism~'(camp_site|caravan_site)']")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select("*[toilets~'.*']")
...
object ToiletsFeeQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select("na[amenity=toilets][access!~'(private|customers)'][!seasonal]")
...
object RoadSurfaceQuest : Quest {
  ...
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features
          .select(
              "w[highway~'(${roadHighways.joinToString("|")})'],w[highway=service][service!~'(driveway|slipway)']")
          .select("*[access!~'(private|no)'],*[foot][foot!=private]")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select("*[surface]")
...
object SidewalkSurfaceQuest : Quest {
  ...
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features
          ...
          .select(
              "*[sidewalk~'(both|left|right)'],*[sidewalk:both=yes],*[sidewalk:left=yes][sidewalk:right~'(yes|no|separate)'],*[sidewalk:right=yes][sidewalk:left~'(yes|no|separate)']")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "*[sidewalk:both:surface],*[sidewalk:left:surface],*[sidewalk:right:surface],*[sidewalk:surface]")
...
object PathSurfaceQuest : Quest {
  ...
  private val SOFT_SURFACES =
      setOf("ground", "earth", "dirt", "grass", "sand", "mud", "ice", "salt", "snow", "woodchips")

  private val ANYTHING_UNPAVED =
      SOFT_SURFACES +
          setOf(
              "unpaved",
              "compacted",
  ...
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "w[highway~'(${sidewalkHighways.joinToString("|")})'][segregated!=yes][access!~'(private|no)'][!conveying][!indoor]")

  // TODO: .select("*[~'(path|footway|cycleway|bridleway)'!~'link']")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select("*[surface][surface!~'(${ANYTHING_UNPAVED.joinToString("|")})']")
...
object CyclewayPartSurfaceQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features
          .select(
              "w[highway=cycleway],w[highway~'(path|footway)'][bicycle],w[highway=bridleway][bicycle~'(designated|yes)']")
          .select("*[segregated=yes][!sidewalk]")
          .select(
              "*[access!~'(private|no)'],*[foot][foot!~'(private|no)'],*[bicycle][bicycle!~'(private|no)']")

  // TODO: .select("*[~'(path|footway|cycleway|bridleway)'!~'link']")

  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "*[cycleway:surface][cycleway:surface!~'paved|unpaved'],*[cycleway:surface:note],*[note:cycleway:surface]")
...
object FootwayPartSurfaceQuest : Quest {
  override fun allFeatures(features: Features<Feature>): Features<Feature> =
      features
  ...
  override fun solvedFeatures(features: Features<Feature>): Features<Feature> =
      features.select(
          "*[footway:surface][footway:surface!~'(paved|unpaved)'],*[footway:surface:note],*[note:footway:surface]")
...
val quests =
    listOf(
        BikeParkingCapacityQuest,
        BikeParkingTypeQuest,
        BikeParkingCoverQuest,
        BikeRentalCapacityQuest,
        BikeRentalTypeQuest,
        BusStopBenchQuest,
        BusStopBinQuest,
        BusStopLitQuest,
        ...
        ToiletAvailabilityQuest,
        ToiletsFeeQuest,
        RoadSurfaceQuest,
        SidewalkSurfaceQuest,
        PathSurfaceQuest,
        CyclewayPartSurfaceQuest,
        FootwayPartSurfaceQuest,
    )
