...
fun generateData(): OutputData {
  ...
  val cities =
      library.select(
          "a[admin_level=8][population>20000][name!~'(Görlitz|Karviná|Opava|Ostrava|Český Těšín|Trutnov|Zittau|Třinec)']")
...
fun main() {
  ...
  var data: OutputData?
  val generationTime = measureTimeMillis { data = generateData() }
          ...
          renderer.render(
              ...
              commonContext +
                  mapOf(
                      "quest" to quest,
                      "result" to
                          data!!.questResults[quest]!!.toList().sortedByDescending {
                            it.second.solved
                          },
                  ))
...
class ResourceTemplateProvider(private val basePath: String) : TemplateProvider {
  override suspend fun get(template: String): String? {
    return this::class
        .java
        .classLoader
        .getResource(Paths.get(basePath, template).toString())
        ?.readText()
  }
}
